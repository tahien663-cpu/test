...
System: ### Vấn đề
Lỗi `500 (Internal Server Error)` tại endpoint `/api/chat/history` cho thấy có vấn đề trong logic xử lý, có thể liên quan đến Supabase (bảng `chats`/`messages` không tồn tại, quyền truy cập, hoặc lỗi truy vấn). Lỗi `404` ban đầu có thể do sai URL hoặc endpoint không khớp. Frontend gửi request tới `https://test-d9o3.onrender.com/api/chat/history`, nhưng server có vẻ chạy trên domain khác (`https://hein1.onrender.com`).

### Giải pháp
1. **Cập nhật `server.js`**:
   - Thêm logging chi tiết trong `/api/chat/history` để xác định lỗi.
   - Cập nhật `allowedOrigins` để bao gồm `https://test-d9o3.onrender.com`.
   - Đảm bảo endpoint trả về lỗi rõ ràng.

2. **Kiểm tra Supabase**:
   - Tạo bảng `users`, `chats`, `messages`.
   - Tạo hàm RPC `delete_chat_with_messages`.
   - Kiểm tra `SUPABASE_SERVICE_ROLE_KEY`.

3. **Kiểm tra frontend**:
   - Đảm bảo request `/api/chat/history` gửi header `Authorization: Bearer <token>`.
  ...
  try {
    ...
    if (!chatId) {
      ...
      const { data: chat, error: chatError } = await supabase
        .from('chats')
        .insert([{ user_id: userId, title: firstMessage.substring(0, .Вставить
...
System: ### Vấn đề
Lỗi `500 (Internal Server Error)` tại endpoint `/api/chat/history` cho thấy có vấn đề trong logic xử lý, có thể do Supabase (bảng `chats`/`messages` không tồn tại, quyền truy cập, hoặc lỗi truy vấn). Lỗi `404` ban đầu có thể do sai URL. Frontend gửi request tới `https://test-d9o3.onrender.com/api/chat/history`, nhưng server có thể chạy trên `https://hein1.onrender.com`.

### Giải pháp
1. **Cập nhật `server.js`**:
   - Thêm logging chi tiết trong `/api/chat/history` để xác định lỗi.
   - Cập nhật `allowedOrigins` để bao gồm `https://test-d9o3.onrender.com`.
   - Xử lý lỗi Supabase rõ ràng hơn.
2. **Kiểm tra Supabase**:
   - Tạo bảng `users`, `chats`, `messages`.
   - Tạo hàm RPC `delete_chat_with_messages`.
   - Kiểm tra `SUPABASE_SERVICE_ROLE_KEY`.
