    ...
    <div className={`min-h-screen flex flex-col relative overflow-hidden ${theme === 'light' ? 'bg-gray-100' : 'bg-gray-900'}`}>
      ...
      {showAbout && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-40 transition-opacity duration-300 px-4">
          <div className={`backdrop-blur-2xl rounded-3xl p-6 sm:p-8 max-w-lg w-full transform transition-all duration-300 scale-95 animate-[modalIn_0.3s_ease-out_forwards] ${theme === 'light' ? 'bg-white/90' : 'bg-white/[0.08]'}`}>
            <div className="flex items-center gap-3 mb-4">
              <Sparkles className={`w-7 h-7 ${theme === 'light' ? 'text-blue-700' : 'text-blue-400'}`} />
              <h2 className={`text-2xl font-bold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}>About Hein AI</h2>
            </div>
            
            <div className={`space-y-4 ${theme === 'light' ? 'text-gray-900' : 'text-white/90'}`}>
              <p>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Zap className={`w-5 h-5 ${theme === 'light' ? 'text-blue-700' : 'text-blue-400'}`} />
              ...
              <div className="flex items-center gap-2 pt-2">
                <Calendar className={`w-5 h-5 ${theme === 'light' ? 'text-blue-700' : 'text-blue-400'}`} />
                <p>Phiên bản hiện tại: <strong className={theme === 'light' ? 'text-gray-900' : 'text-white'}>v2.1</strong></p>
              </div>
              
              <div className="flex items-center gap-2">
                <User className={`w-5 h-5 ${theme === 'light' ? 'text-blue-700' : 'text-blue-400'}`} />
            ...
            <button
              ...
              className={`mt-6 w-full px-4 py-2 rounded-xl transition-all duration-200 hover:scale-105 font-semibold ${theme === 'light' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-purple-500/20'}`}
            >
              Đóng
            </button>
          </div>
