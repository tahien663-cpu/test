...
export default function Login() {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    if (isRegister && (name.length < 2 || name.length > 50)) {
      setMessage({ text: 'Tên phải từ 2-50 ký tự', type: 'error' });
      return;
    }
    ...
    try {
      ...
      let errorMessage = error.message || 'Không thể kết nối tới server';
  ...
  useEffect(() => {
    ...
    if (token) {
      apiService.getChatHistory().then(() => navigate('/chat')).catch(() => {
        localStorage.removeItem('token');
        localStorage.removeItem('userName');
        localStorage.removeItem('userEmail');
      });
